...
REALM_NAME="${KEYCLOAK_REALM:-familytree}"
API_CLIENT_ID="${KEYCLOAK_API_CLIENT_ID:-familytree-api}"
CLI_CLIENT_ID="${KEYCLOAK_CLI_CLIENT_ID:-familytree-cli}"
WEB_CLIENT_ID="${KEYCLOAK_WEB_CLIENT_ID:-familytree-web}"

echo "🔧 Initializing Keycloak for Family Tree..."
...
if [ "$REALM_EXISTS" = "false" ]; then
    ...
    curl -sf -X POST "$KEYCLOAK_URL/admin/realms" \
        ...
        -d "{
            ...
            \"displayNameHtml\": \"<b>Family Tree</b> Management\",
...
API_CLIENT_JSON=$(cat <<EOF
{
    ...
    "description": "Family Tree HTTP API Server",
    "enabled": true,
...
CLI_CLIENT_JSON=$(cat <<EOF
{
    ...
    "name": "Family Tree CLI",
    "description": "Family Tree Command Line Interface",
    "enabled": true,
    "protocol": "openid-connect",
    "publicClient": true,
    "bearerOnly": false,
...
TEST_EMAIL="${TEST_EMAIL:-testuser@familytree.local}"
