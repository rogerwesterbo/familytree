...
This guide explains how to set up OpenID Connect authentication for the Family Tree web application using Keycloak with PKCE.
...
1. Open Keycloak Admin Console: http://localhost:15101
2. Login with admin credentials (admin/admin)
3. Select the `familytree` realm
...
- Root URL: `http://localhost:15200`
- Home URL: `http://localhost:15200`
- Valid redirect URIs: `http://localhost:15200/callback`
- Valid post logout redirect URIs: `http://localhost:15200`
- Web origins: `http://localhost:15200`
- Click **Save**
...
Create a `.env` file in `web/familytreeweb/`:

```bash
VITE_KEYCLOAK_URL=http://localhost:15101
VITE_KEYCLOAK_REALM=familytree
VITE_KEYCLOAK_CLIENT_ID=familytree-web
VITE_REDIRECT_URI=http://localhost:15200/callback
VITE_POST_LOGOUT_REDIRECT_URI=http://localhost:15200
```

## 🚀 Running the Application

```bash
cd web/familytreeweb
npm install
npm run dev
```

Visit: http://localhost:15200
...
window.location.href = `${keycloakURL}/auth?
  client_id=familytree-web&
  redirect_uri=http://localhost:15200/callback&
  response_type=code&
...
export async function logout() {
  ...
  window.location.href = `${keycloakURL}/logout?
    client_id=familytree-web&
    post_logout_redirect_uri=http://localhost:15200&
...
export async function apiClient(url: string, options = {}) {
  ...
  return fetch(`http://localhost:15000${url}`, {
    ...options,
    headers: {
      Authorization: `Bearer ${token}`,
...
- **Role**: `familytree-admin`

### Manual Testing

1. Start services: `docker-compose up -d`
2. Start web app: `cd web/familytreeweb && npm run dev`
3. Open: http://localhost:15200
4. Click "Login"
...
- Ensure `http://localhost:15200/callback` is in Keycloak's "Valid redirect URIs"
- Check for trailing slashes
- Verify port numbers match

### "CORS error"

Update Family Tree API CORS settings in `.env`:

```bash
HTTP_API_CORS_ALLOWED_ORIGINS=http://localhost:15000,http://localhost:15200
```

### "Token verification failed"

- Verify Keycloak URL in `.env` matches running Keycloak
- Check realm name is correct (`familytree`)
- Ensure client ID is `familytree-web`
