...
The Family Tree web application uses a custom theme system built on top of Radix UI's theming capabilities. Dark mode is the default theme.
...
function AppContent() {
  ...
  return <Theme appearance={theme}>{/* App content */}</Theme>;
}
```
...
function ThemeAwareComponent() {
  ...
  return <div className={`component ${theme}`}>{/* Content */}</div>;
}
```
