...
function recreateAll {
  ...
  openssl req -x509 -new -key ca.key -days 3650 -sha256 -subj "/CN=FamilyTree-Local-CA" -out ca.crt

  openssl genrsa -out server.key 4096
  openssl req -new -key server.key -subj "/CN=FamilyTree-Server" -out server.csr
