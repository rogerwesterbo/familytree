...
type TokenCache struct {
	AccessToken  string    `json:"access_token"`  // #nosec G117 -- OAuth2 token field, not a hardcoded secret
	RefreshToken string    `json:"refresh_token"` // #nosec G117 -- OAuth2 token field, not a hardcoded secret
...
type TokenResponse struct {
	AccessToken  string `json:"access_token"`  // #nosec G117 -- OAuth2 token response field, not a hardcoded secret
	RefreshToken string `json:"refresh_token"` // #nosec G117 -- OAuth2 token response field, not a hardcoded secret
	ExpiresIn    int    `json:"expires_in"`
	TokenType    string `json:"token_type"`
...
func GetTokenCachePath() (string, error) {
	...
	cacheDir := filepath.Join(homeDir, ".familytree")
...
func GetValidToken(ctx context.Context) (string, error) {
	...
	return "", fmt.Errorf("no valid token found, please run 'familytreeCLI login' first")
}
